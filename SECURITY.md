...
Afin de prévenir les abus, le spam, les tentatives de force brute et les attaques par déni de service :

- L'ensemble des points d'accès (endpoints) de l'API est protégé par des limiteurs de débit basés sur l'adresse IP du client (`REMOTE_ADDR`).
- Des seuils distincts et restrictifs sont appliqués selon que la requête est en lecture ou en écriture.
