...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Audit de {total_source_count} dossiers par rapport à la base de données locale des utilisateurs...\n"
        )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Utilisateurs correspondants trouvés dans Django : {matched_count}/{total_source_count}"
            )
        )
        ...
        if unmatched_records:
            ...
            self.stdout.write(
                f"{'Dossier DN':<15} | {'Adresse e-mail de l’usager':<35} | {'Problème/Raison':<30}"
            )
            self.stdout.write("-" * 80)
            for item in unmatched_records:
                self.stdout.write(
                    f"{item['dossier']:<15} | {item['email']:<35} | {item['reason']:<30}"
                )
            self.stdout.write("-" * 80)
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    "✓ Correspondance parfaite ! Tous les dossiers DN ont un utilisateur local Django correspondant."
                )
            )
        self.stdout.write("\n")
