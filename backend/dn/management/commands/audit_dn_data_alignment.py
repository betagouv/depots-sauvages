...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for idx, lean_node in enumerate(lean_dossiers, start=1):
            ...
            self.stdout.write(f"[{idx}/{total_source_count}] Analyse du dossier #{dossier_number}...")
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"  Erreur lors de la récupération pour #{dossier_number} : {e}"))
                continue
            ...
            for field_name, expected_val in expected_data.items():
                ...
                norm_expected = (expected_val or "").strip() if isinstance(expected_val, str) else expected_val
                norm_db = (db_val or "").strip() if isinstance(db_val, str) else db_val

                if norm_expected != norm_db:
                    # Role normalization check
                    if field_name == "constatant_role" and norm_db == "president ECPI" and norm_expected and ("président de l" in norm_expected.lower()):
                        continue
        ...
        self.stdout.write(f"Dossiers totalement alignés : {fully_aligned_count}/{total_source_count}")
        ...
        if alignment_discrepancies:
            ...
            self.stdout.write(f"{'Dossier':<10} | {'Champ concerné':<30} | {'Valeur attendue (API DN)':<35} | {'Valeur réelle (Base de données)':<35}")
            self.stdout.write("-" * 120)
            for item in alignment_discrepancies:
                self.stdout.write(f"#{item['dossier']:<9} | {item['field']:<30} | {item['api_val']:<35} | {item['db_val']:<35}")
            self.stdout.write("-" * 120)
            self.stdout.write(self.style.ERROR("Audit d'alignement terminé : des différences de données ou des dossiers manquants ont été repérés."))
        else:
