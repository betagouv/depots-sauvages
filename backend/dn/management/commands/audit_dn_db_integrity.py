...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"  - Suivis sans Constatation : {suivi_sans_constat} (normal pour d'anciens signalements hors DN)")
        ...
        sample_constatations = Constatation.objects.filter(suivi_procedure__signalement__isnull=False)[:5]
        if sample_constatations.exists():
            ...
            for c in sample_constatations:
                ...
                self.stdout.write(f"  - Dossier #{dnsig.dn_numero_dossier} | Commune : {c.commune} | Créé le : {c.created}")
        
        self.stdout.write(self.style.WARNING("\n=== FIN DE L'AUDIT ==="))
        if errors_found:
            self.stdout.write(self.style.ERROR("L'audit a révélé des anomalies de liaisons de données."))
        else:
            self.stdout.write(self.style.SUCCESS("Audit terminé avec succès. Aucune anomalie détectée."))
