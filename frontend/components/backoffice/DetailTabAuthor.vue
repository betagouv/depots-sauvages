    ...
    <div class="fr-grid-row fr-grid-row--gutters">
      <div class="fr-col-6">
        ...
        <p class="fr-text--md fr-mb-1w">
          <strong>{{ procedure.auteur_identifie ? 'Oui' : 'Non' }}</strong>
        </p>
      </div>
      ...
      <div class="fr-col-12" v-if="procedure.auteur_nom || procedure.auteur_prenom">
        ...
        <p class="fr-text--md fr-mb-1w">
          <strong
            >{{ procedure.auteur_civilite }} {{ procedure.auteur_prenom }}
            {{ procedure.auteur_nom }}</strong
          >
          <span v-if="procedure.auteur_siret"> (SIRET: {{ procedure.auteur_siret }})</span>
          <span
            v-if="procedure.entreprise_francaise !== null"
            class="fr-text--xs fr-text-mention--grey"
          >
      ...
      <div class="fr-col-12 bo-dashed-separator">
        ...
        <div class="fr-mt-1v fr-mb-1w bo-flex-row-wrap">
          <span v-for="idx in procedure.indices_disponibles" :key="idx" class="fr-tag fr-tag--sm">
            {{ idx }}
          </span>
          <span v-if="!procedure.indices_disponibles?.length" class="fr-text-mention--grey"
            >Aucun indice</span
          >
        </div>
